...
import '../services/Api/delivery_reports_service.dart';
import '../models/delivery_reports_model.dart';
import '../services/pdf_service.dart';

class ReportDelevery extends StatefulWidget {
  final String phone;

  const ReportDelevery({super.key, required this.phone});

  @override
  State<ReportDelevery> createState() => _ReportDeleveryState();
}

class _ReportDeleveryState extends State<ReportDelevery> {
  final DeliveryReportsService _reportsService = DeliveryReportsService();

  // حالة التحميل والبيانات
  DeliveryReportsLoadingState _loadingState = DeliveryReportsLoadingState.initial;
  DeliveryReportsModel? _reportsData;
  DeliveryReportsError? _error;
  ...
  Future<void> _loadReports() async {
    setState(() {
      _loadingState = DeliveryReportsLoadingState.loading;
      _error = null;
    });

    try {
      final response = await _reportsService.getMyDeliveryReports(
      ...
      if (response.statusCode == 200) {
        final reportsModel = DeliveryReportsModel.fromJson(response.data);
        setState(() {
          _reportsData = reportsModel;
          _loadingState = DeliveryReportsLoadingState.loaded;
        });
      } else {
      ...
      setState(() {
        _error = DeliveryReportsError.fromException(e);
        _loadingState = DeliveryReportsLoadingState.error;
      });
      debugPrint('خطأ في جلب تقارير السائق: $e');
    }
  }
  ...
  Future<void> _exportToPDF() async {
    ...
    try {
      ...
      // إنشاء PDF
      final pdfBytes = await PDFService.generateDeliveryReportPDF(
      ...
      // إنشاء اسم الملف
      final fileName = _generateFileName('تقرير_السائق');
  ...
  Widget _buildContent() {
    switch (_loadingState) {
      case DeliveryReportsLoadingState.loading:
        return const Center(
          child: Padding(
      ...
      case DeliveryReportsLoadingState.error:
        return Center(
          child: Padding(
      ...
      case DeliveryReportsLoadingState.loaded:
